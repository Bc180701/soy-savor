<?php
    ...
    define("PRINT_QUEUE_DIR", "print-queue-chateaurenard");
    define("PRINT_ARCHIVE_DIR", "print-archive-chateaurenard");
    ...
    if ($http_request == 'GetRequest') {
        ...
        // Header
        $xml .= '<!--  En-tête  -->' . "\n";
        $xml .= '<text align="center" width="2" height="2" em="true">SUSHI EATS CHÂTEAURENARD</text>' . "\n";
        $xml .= '<feed line="1"/>' . "\n";
        ...
        foreach ($order['items'] as $item) {
            ...
            $xml .= '<text width="2" height="2">' . $itemQuantity . 'x ' . $itemName . '</text>' . "\n";
            $xml .= '<feed line="1"/>' . "\n";
        ...
        if ($version == '3.00') {
            // Log result
            $fhandle = @fopen("ResultPrint_chateaurenard.log", "wt");
