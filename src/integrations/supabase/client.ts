...
// Helper function to check if a postal code is in a delivery area
export const isPostalCodeInDeliveryArea = async (postalCode: string): Promise<boolean> => {
  try {
    const { data, error } = await supabase
    ...
    console.error("Exception checking postal code:", error);
    return false;
  }
};

// Helper function to get all delivery locations
export const getDeliveryLocations = async () => {
  try {
    ...
    console.error("Exception fetching delivery locations:", error);
    return [];
  }
};
