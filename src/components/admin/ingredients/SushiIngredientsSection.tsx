...
const SushiIngredientsSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        if (error) throw error;
        toast.success("Ingrédient sushi modifié avec succès");
      } else {
        const { error } = await supabase
          .from('sushi_ingredients')
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Êtes-vous sûr de vouloir supprimer cet ingrédient sushi ?")) {
      return;
    }
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Ingrédient sushi supprimé avec succès");
      fetchIngredients();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {editingId ? "Modifier l'ingrédient sushi" : "Ajouter un ingrédient sushi"}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="sushi-name">Nom de l'ingrédient</Label>
                <Input
                  id="sushi-name"
                  value={formData.name}
              ...
              <div>
                <Label htmlFor="sushi-price">Prix (€)</Label>
                <Input
                  id="sushi-price"
                  type="number"
                  step="0.01"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="sushi-type">Type d'ingrédient</Label>
                <Select
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="sushi-included">Inclus de base</Label>
              </div>
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Ingrédients sushi existants ({ingredients.length})</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {ingredients.length === 0 && (
            <p className="text-center text-gray-500 py-8">
              Aucun ingrédient sushi trouvé. Ajoutez-en un pour commencer.
            </p>
          )}
        </CardContent>
      </Card>
...
export default SushiIngredientsSection;
