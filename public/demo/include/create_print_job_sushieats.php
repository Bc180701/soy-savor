...
function respond_sushi($payload, $code = 200) {
...
try {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        respond_sushi(['ok' => false, 'error' => 'Method not allowed'], 405);
    }

    $body = json_input_sushi();
    $token = isset($body['token']) ? (string)$body['token'] : '';
    if ($token !== $EXPECTED_TOKEN) {
        respond_sushi(['ok' => false, 'error' => 'Forbidden'], 403);
    }
    ...
    if (!is_dir($dir)) {
        if (!mkdir($dir, 0755, true)) {
            respond_sushi(['ok' => false, 'error' => 'Failed to create directory'], 500);
        }
    }
    $written = file_put_contents($targetPath, implode("\n", $xml));
    if ($written === false) {
        respond_sushi(['ok' => false, 'error' => 'Failed to write file'], 500);
    }

    respond_sushi(['ok' => true, 'file' => basename($targetPath)]);
} catch (Throwable $e) {
    respond_sushi(['ok' => false, 'error' => $e->getMessage()], 500);
}
