<?php
// Endpoint dedicated to SoySavor printer: writes to ../request/sample_soysavor.xml
header('Content-Type: application/json; charset=utf-8');

$EXPECTED_TOKEN = getenv('EPSON_PRINT_TOKEN') ?: 'CHANGE_ME_SECRET';
$TARGET_FILE_RELATIVE = '../request/sample_soysavor.xml';

function json_input_soy() {
    $raw = file_get_contents('php://input');
    $data = json_decode($raw, true);
    return is_array($data) ? $data : [];
}

function respond_soy($payload, $code = 200) {
...
try {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        respond_soy(['ok' => false, 'error' => 'Method not allowed'], 405);
    }

    $body = json_input_soy();
    $token = isset($body['token']) ? (string)$body['token'] : '';
    if ($token !== $EXPECTED_TOKEN) {
        respond_soy(['ok' => false, 'error' => 'Forbidden'], 403);
    }
    ...
    $xml[] = '<text lang="fr" align="center" width="2" height="2">Soy Savor</text>';
    ...
    if (!is_dir($dir)) {
        if (!mkdir($dir, 0755, true)) {
            respond_soy(['ok' => false, 'error' => 'Failed to create directory'], 500);
        }
    }
    $written = file_put_contents($targetPath, implode("\n", $xml));
    if ($written === false) {
        respond_soy(['ok' => false, 'error' => 'Failed to write file'], 500);
    }

    respond_soy(['ok' => true, 'file' => basename($targetPath)]);
} catch (Throwable $e) {
    respond_soy(['ok' => false, 'error' => $e->getMessage()], 500);
}
