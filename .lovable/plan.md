...
1. **Stripe Terminal (recommandé)** — Stripe est déjà en place sur le projet (clés par restaurant, webhook). La borne envoie le montant au lecteur (BBPOS WisePOS E ou Verifone P400, filaire/Ethernet), le client insère sa carte, la confirmation revient dans le code : impression automatique, zéro saisie manuelle, réconciliation propre avec la commande. C'est la seule option où la borne est réellement autonome.
