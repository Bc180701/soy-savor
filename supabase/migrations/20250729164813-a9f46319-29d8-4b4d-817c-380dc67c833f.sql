-- Partie 2 corrigée: Créer la table admin_permissions et les fonctions
...
FOR ALL 
USING (has_role(auth.uid(), 'super_administrateur'));
...
AS $$
  ...
  SELECT CASE 
    WHEN has_role(user_id, 'super_administrateur') THEN true
...
BEGIN
  ...
  IF anais_id IS NOT NULL THEN
    ...
    VALUES (anais_id, 'super_administrateur')
    ON CONFLICT (user_id, role) DO NOTHING;
  END IF;
  
  IF clweb_id IS NOT NULL THEN
    INSERT INTO public.user_roles (user_id, role) 
    VALUES (clweb_id, 'super_administrateur')
    ON CONFLICT (user_id, role) DO NOTHING;
  END IF;
END $$;